...
- **Alembic migrations are always autogenerated, never hand-written.** Change the SQLAlchemy models, then run `uv run alembic revision --autogenerate -m "NNNN <summary>"` and let Alembic diff the models against the DB. Do not author or hand-edit the `op.*` calls in a version file. If autogenerate produces something wrong, fix the *models* (or the naming convention / `env.py`) and regenerate — don't patch the generated file.
- Autogenerate emits schema DDL only, not data backfills. Keep migrations pure-schema; this project is greenfield with no persisted data to migrate. If a populated table ever needs a data migration, raise it explicitly rather than silently hand-editing a version file.
- After generating: `uv run ruff format alembic/versions/<file>.py`, then verify `uv run alembic upgrade head` and `uv run alembic downgrade -1` round-trip.
