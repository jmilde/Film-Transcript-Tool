...
- [x] `cd frontend && npm run lint && npm run typecheck && npm run test && npm run build` all clean — 188/188 tests pass
- [x] `cd backend && make check` green (Phase 5's touch) — `ruff check`/`ruff format --check`/`mypy --strict` all clean; `pytest -m "not integration"`: 356/357 passed. The one failure, `test_exports.py::test_build_document_resolves_speaker_name`, is a pre-existing test bug unrelated to this overhaul (confirmed by reading the code, not just re-running it): the test's own verification query (`select(Speaker).where(Speaker.provider_identifier == "speaker_0").scalar_one()`) has no scoping to the transcript/video under test, so it breaks whenever the shared dev Supabase instance has *any* other committed `Speaker` row named `"speaker_0"` — which real Deepgram diarization produces for every processed video. `build_export_document`'s actual service-code lookup (`app/services/exports.py`) is correctly scoped by `speaker.id`; only the test's own assertion query is at fault. Reproduces in isolation, not touched by Phase 5 (`video.py` schema/route) or anything frontend
- [x] Manual pass, both themes: sign in → Projects → create/open a project → browse folders → open a video → select transcript text → add a comment → open the document panel → add a clip → format text → open Search (⌘F) from inside the video workspace → click a hit → confirm "Back to search" → open Ask from the header → ask a question → click a citation → confirm "Back to chat" → toggle the theme at various points and confirm nothing is illegible in either mode — walked the full flow via Playwright: added a real comment and a real clip to the project's document (both persisted correctly through a reload), confirmed the formatting toolbar/Select/Popover primitives all render correctly (already covered in depth in Phase 6/7/9/11's per-phase verification, including a reproduced-and-clean-recovered document-conflict-banner sighting), Search/Ask/Back-to-search/Back-to-chat all confirmed working in Phases 6-9. Sign-in checked via a markup-accurate DOM overlay (Phase 12) rather than the live route, since the browser session was already authenticated and signing out would need credentials nobody supplied
- [x] Confirm no leftover references to the deleted `SearchPage`/`icons.tsx`/`slate-*` ad hoc classes (a final grep for `slate-` and `text-slate` across `frontend/src` should turn up nothing outside of anything intentionally left, e.g. transitional third-party class names) — found and fixed one real leftover (`auth/RequireAuth.tsx`'s loading state, missed because Phases 10/11 never had reason to open `auth/`) plus tightened two stale code comments that named the deleted `SearchPage` without qualifying it as historical (`store/documentPanel.ts`, `pages/VideoWorkspace.tsx`); left `SearchCommandPalette.tsx`'s "ported from the old `SearchPage` route" comment as-is since it's self-explanatory provenance, not a live reference. Final `grep -rE "slate-|gray-|zinc-|neutral-|amber-|violet-|red-|sky-|teal-|orange-|yellow-|emerald-"` across all of `frontend/src` is clean
