...
- [x] Verify: pytest (114 passed), mypy clean, ruff check + format clean; `alembic upgrade head`/`downgrade -1` round-trip on real Supabase; **real speech clip driven end-to-end against real Supabase + real ffmpeg — all 4 media stages completed, transcribe reached the live Deepgram endpoint and formed a valid request**. Live transcription itself is **deferred**: `.env` has `DEEPGRAM_API_KEY=placeholder` (Deepgram returned a structured 401 INVALID_AUTH), so no real API key is provisioned yet. Drop a real key into `.env` and re-drive to complete the live transcript/speaker-rename check; normalize + service + handler + routes are fully covered by tests against the saved Deepgram fixture and the real (rolled-back) DB.
